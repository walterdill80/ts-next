    ...
    <section className="py-24 lg:py-32">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[var(--ts-heading)]">
        ...
        <div className="space-y-2">
          {faqs.map((faq) => (
            <details
              key={faq.question}
              className="ts-faq-item overflow-hidden rounded-xl border border-[var(--ts-border-base)] bg-[var(--ts-surface-card)]"
            >
              <summary className="flex list-none cursor-pointer items-center justify-between gap-4 px-5 py-4 text-left">
                <span className="text-sm font-medium text-[var(--ts-text-strong)]">
                  {faq.question}
                </span>
                <ChevronDown className="faq-chevron w-4 h-4 flex-shrink-0 text-[var(--ts-text-subtle)] transition-transform duration-300" />
              </summary>
              <p className="px-5 pb-5 text-sm text-[var(--ts-text-muted)] leading-relaxed">
                {faq.answer}
              </p>
            </details>
          ))}
